...
color ray_color(const ray& r, const shared_ptr<texture>& background, const shared_ptr<pdf>& background_pdf, const hittable& world, const shared_ptr<hittable>& lights, int depth) 
{
	...
	srec.attenuation * rec.mat_ptr->scattering_pdf(r, rec, scattered)
			 * ray_color(scattered, background, background_pdf, world, lights, depth-1) / pdf_val)*color((int)(c_r*255),(int)(c_g*255),(int)(c_b*255));
}
...
color render_samples(int x, int y, int w, int h) {
	...
	for (int i = 0; i < samples_per_pixel; i++) {
		...
		pixel += ray_contrib*color(c_r,c_g,c_b);
	}
...
*/
		...
		for(int y=0;y<h;y+=2) 
		{
			for(int x=0;x<w;x+=1)
			{
				...
				int r2 = clamp(pow(prev[i].x()*scale, inv_gamma),0.0,0.999)*256; 
				int g2 = clamp(pow(prev[i].y()*scale, inv_gamma),0.0,0.999)*256; 
				int b2 = clamp(pow(prev[i].z()*scale, inv_gamma),0.0,0.999)*256; 
				raytraced[i+w] = MAKE_RGB((r+r2)/3,(g+g2)/3,(b+b2)/3);
				prev[i] = pixel;
			}
		}
